  ...
  <div class="max-w-3xl mx-auto p-8 space-y-10">
    ...
    <section class="space-y-3">
      ...
      <kb-accordion-group>
        ...
        <kb-accordion>
          <span slot="trigger">How does it work with Vue?</span>
          Through native custom element support in Vue 3 with the isCustomElement compiler option and .prop modifier for complex data.
        </kb-accordion>
        <kb-accordion>
    ...
    <section class="space-y-3">
      ...
      <kb-tabs variant="line" size="md">
        <span v-for="(t, i) in TABS_CONFIG" :key="`tab-${i}`" :slot="`tab-${i}`">{{ t.label }}</span>
        <div v-for="(t, i) in TABS_CONFIG" :key="`panel-${i}`" :slot="`panel-${i}`" class="p-4 text-sm text-slate-600 dark:text-zinc-300">
          {{ t.panel }}
        </div>
      </kb-tabs>
    </section>
    ...
    <section class="space-y-3">
      ...
      <div class="flex flex-wrap gap-2">
        <kb-tag v-for="v in TAG_VARIANTS" :key="v" :variant="v">{{ v }}</kb-tag>
        <kb-tag v-for="s in TAG_SIZES" :key="s" :size="s" variant="outline">{{ s }}</kb-tag>
        <kb-tag variant="subtle" closable @kb-close="showAlert('tag removed')">
          Closable
        </kb-tag>
      </div>
      <kb-tag-group>

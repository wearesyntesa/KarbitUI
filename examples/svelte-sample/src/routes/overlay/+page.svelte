...
<div class="max-w-2xl mx-auto p-8 space-y-10">
  ...
  <section class="space-y-3">
    ...
    <div class="flex gap-2">
      <kb-button variant="outline" onclick={() => (alertOpen = true)}>Confirm action</kb-button>
      <kb-button variant="outline" color-scheme="red" onclick={() => (deleteAlertOpen = true)}>Delete item</kb-button>
    </div>
    <kb-alert-dialog
      open={alertOpen}
      onkb-confirm={() => (alertOpen = false)}
      onkb-cancel={() => (alertOpen = false)}
    >
    ...
    <kb-alert-dialog
      ...
      onkb-confirm={() => (deleteAlertOpen = false)}
      onkb-cancel={() => (deleteAlertOpen = false)}
    >
